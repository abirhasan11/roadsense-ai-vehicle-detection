  ...
  return (
    ...
    >
      ...
      <div className={`w-full mx-auto space-y-4 ${isPhoneFrame ? 'p-4 sm:p-5 max-w-2xl' : 'p-4 sm:p-6 lg:p-8 max-w-[1600px]'}`}>
        ...
        {record.isAutonomous ? (
          <div className="p-4 rounded-2xl bg-emerald-50 dark:bg-emerald-950/40 border border-[#1FAE71]/40 text-emerald-950 dark:text-emerald-200 flex items-center justify-between shadow-xs">
            ...
            <div className="hidden sm:flex items-center gap-2">
              <span className="text-xs font-mono bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 px-3 py-1 rounded-full font-bold">
                Hardware Verified
              </span>
            </div>
          </div>
        ) : (
          <div className="p-4 rounded-2xl bg-red-50 dark:bg-red-950/40 border border-[#E5484D]/40 text-red-950 dark:text-red-200 flex items-center justify-between shadow-xs">
            <div className="flex items-center gap-3">
            ...
            <div className="hidden sm:flex items-center gap-2">
              <span className="text-xs font-mono bg-red-500/20 text-red-600 dark:text-red-400 px-3 py-1 rounded-full font-bold">
                Human-Driven
              </span>
            </div>
          </div>
        ...
        {/* 2-Column Responsive Grid on lg screens in full window, single column in phone frame */}
        <div className={isPhoneFrame ? "flex flex-col space-y-4 max-w-2xl mx-auto w-full" : "grid grid-cols-1 lg:grid-cols-12 gap-5 items-start"}>
          {/* Left Column (7 cols on lg): Visual Scene, Heatmap & Timeline */}
          <div className={isPhoneFrame ? "w-full space-y-3.5" : "lg:col-span-7 space-y-3.5"}>
            ...
            <div className="flex flex-wrap items-center justify-between gap-2 bg-slate-200 dark:bg-slate-900 p-1.5 rounded-2xl border border-slate-300 dark:border-slate-800 text-xs">
              ...
              {/* Explainable AI (Grad-CAM) Heatmap Toggle */}
              <button
                onClick={() => {
            ...
            <div className="relative w-full h-64 sm:h-72 lg:h-[340px] rounded-3xl overflow-hidden border border-[#E4E0FD] dark:border-slate-800 bg-slate-950 shadow-md">
              {viewMode === 'standard' ? (
                <RoadSceneSVG
              ...
              {/* Explainable AI Grad-CAM Saliency Overlay */}
              {showExplainableHeatmap && (
            ...
            {/* AI Confidence Convergence Timeline Chart */}
          ...
          {/* Right Column (5 cols on lg in full window): Gauge, Metrics, Risk, Metadata & Actions */}
          <div className={isPhoneFrame ? "w-full space-y-3.5" : "lg:col-span-5 space-y-3.5"}>
            {/* Circular Gauge Score + Metric Grid */}
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 items-center">
              {/* Circular Progress Gauge */}
              <div className="p-3.5 rounded-3xl bg-white dark:bg-slate-900 border border-[#E4E0FD] dark:border-slate-800 shadow-xs flex flex-col items-center justify-center gap-1.5">
                <ConfidenceRing
              ...
              <div className="sm:col-span-2 grid grid-cols-2 gap-2.5">
                <div className="p-3 rounded-2xl bg-white dark:bg-slate-900 border border-[#E4E0FD] dark:border-slate-800 shadow-xs">
                  <p className="text-[10px] uppercase font-semibold text-slate-400">Latency</p>
                  <p className="text-base font-bold font-space text-[#5A41DE] mt-0.5">{record.processingTimeMs} ms</p>
                </div>

                <div className="p-3 rounded-2xl bg-white dark:bg-slate-900 border border-[#E4E0FD] dark:border-slate-800 shadow-xs">
                  <p className="text-[10px] uppercase font-semibold text-slate-400">{getTranslation(lang, 'sensorState')}</p>
                  <p className="text-xs font-bold font-space text-slate-800 dark:text-slate-200 mt-1 truncate">{record.lidarStatus}</p>
                </div>

                <div className="p-3 rounded-2xl bg-white dark:bg-slate-900 border border-[#E4E0FD] dark:border-slate-800 shadow-xs col-span-2">
                  <p className="text-[10px] uppercase font-semibold text-slate-400">{getTranslation(lang, 'estDistance')}</p>
                  <p className="text-sm font-bold font-space text-slate-800 dark:text-slate-200 mt-0.5">
            ...
            {/* Predictive AI Risk Assessment Card */}
            <RiskAssessmentCard
              isAutonomous={record.isAutonomous}
              location={record.location}
            />
            ...
            <div className="bg-white dark:bg-slate-900 rounded-3xl p-4 border border-[#E4E0FD] dark:border-slate-800 shadow-xs space-y-3">
              <h3 className="font-space font-bold text-slate-900 dark:text-white text-sm border-b border-slate-100 dark:border-slate-800 pb-1.5">
              ...
              <div className="space-y-2 text-xs">
                <div className="flex items-center justify-between py-1 border-b border-slate-100 dark:border-slate-800">
                  <span className="text-slate-500 dark:text-slate-400 flex items-center gap-1.5">
                    <ShieldCheck className="w-3.5 h-3.5 text-[#5A41DE]" />
                ...
                <div className="flex items-center justify-between py-1 border-b border-slate-100 dark:border-slate-800">
                  <span className="text-slate-500 dark:text-slate-400 flex items-center gap-1.5">
                    <Cpu className="w-3.5 h-3.5 text-[#5A41DE]" />
                ...
                <div className="flex items-center justify-between py-1 border-b border-slate-100 dark:border-slate-800">
                  <span className="text-slate-500 dark:text-slate-400 flex items-center gap-1.5">
                    <Clock className="w-3.5 h-3.5 text-[#5A41DE]" />
                ...
                <div className="flex items-center justify-between py-1 border-b border-slate-100 dark:border-slate-800">
                  <span className="text-slate-500 dark:text-slate-400 flex items-center gap-1.5">
                    <MapPin className="w-3.5 h-3.5 text-[#5A41DE]" />
                ...
                <div className="flex items-center justify-between py-1">
                  <span className="text-slate-500 dark:text-slate-400 flex items-center gap-1.5">
                    <Radio className="w-3.5 h-3.5 text-[#1FAE71]" />
            ...
            <div className="space-y-2.5 pt-1">
              <div className="grid grid-cols-2 gap-2.5">
                <button
                  ...
                  className="py-2.5 px-3 rounded-2xl bg-white dark:bg-slate-900 border border-[#5A41DE] text-[#5A41DE] hover:bg-violet-50 dark:hover:bg-violet-950/40 font-semibold text-xs flex items-center justify-center gap-1.5 transition-all cursor-pointer shadow-xs"
                >
                ...
                <button
                  ...
                  className="py-2.5 px-3 rounded-2xl bg-white dark:bg-slate-900 border border-[#E4E0FD] dark:border-slate-800 text-slate-700 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800 font-semibold text-xs flex items-center justify-center gap-1.5 transition-all cursor-pointer shadow-xs"
                >
              ...
              <button
                ...
                className="w-full py-3 px-6 rounded-2xl bg-[#5A41DE] hover:bg-[#4E35CD] text-white font-bold text-sm shadow-md transition-all flex items-center justify-center gap-2 cursor-pointer active:scale-[0.99]"
              >
